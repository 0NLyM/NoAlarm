...
- **Doppio meccanismo attivo in parallelo**: il notification bridging (v1.4.36, sotto) resta com'era — `NotificationHelper.ringing()` continua a essere postata con `setLocalOnly(!alarm.ringOnWatch)` indipendentemente dalla Data Layer. Se `connectedNodes` risulta vuoto come temuto, il notification bridging resta l'unico altro tentativo attivo; se invece la Data Layer funziona per davvero su questo hardware, i due sono ridondanti ma non in conflitto (il watch riceverebbe sia il messaggio via `RingListenerService` sia, se il bridging funzionasse, anche la notifica bridgeata).
- **Non ancora confermato se la Data Layer funzioni per davvero su questo hardware** — l'utente ha pero' confermato (v1.4.36) che il bridging di notifiche in generale funziona ancora (vedi Causa 13 sotto): non e' chiaro se anche i messaggi Data Layer arrivino, dato che i due canali sono indipendenti e finora e' stato segnalato solo l'esito della notifica.
...
12. **Causa 12 (v1.4.34, causa reale — architettura di notification bridging del v1.4.26)**: la notifica sveglia e' anche l'argomento di `ServiceCompat.startForeground(..., FOREGROUND_SERVICE_TYPE_MEDIA_PLAYBACK)` — le notifiche "media playback" sono l'eccezione esplicita alla modifica Android 14 che rende dismissibili le notifiche dei foreground service, quindi restano *ongoing* (`FLAG_ONGOING_EVENT`) qualunque cosa imposti il `Builder` (per questo togliere `setOngoing(true)` in v1.4.33 non bastava). La pagina ufficiale Wear OS ("Bridging options for notifications") esclude dal bridging esattamente 4 casi: `setLocalOnly(true)`, `setOngoing`/`FLAG_ONGOING_EVENT`, `FLAG_NO_CLEAR`, bridging disattivato dall'app watch — i primi tre erano gia' verificati a posto sul `Builder`, ma il terzo veniva reintrodotto dal sistema a valle di `startForeground`, non dal codice Kotlin.
    - **Fix**: due notifiche distinte in `NotificationHelper.kt` — `foregroundPlaceholder()` (id `ID_RINGING_FGS`, lega il foreground service, sempre `setLocalOnly(true)`, porta lo schermo intero sul telefono) e `ringing()` (id `ID_RINGING`, postata con `NotificationManagerCompat.notify()` senza mai passare da `startForeground`, mai ongoing, unica candidata al bridging, governata da `setLocalOnly(!alarm.ringOnWatch)` come sempre).
   - **Prova reale (v1.4.36, utente su TicWatch E3): non bastava neanche questo.** Fatto nuovo e decisivo: `showAlarmReminder()` (preavviso, canale `CH_UPCOMING`, nessun `setCategory`) arriva regolarmente sul watch, mentre `ringing()` (canale `CH_ALARM`, `CATEGORY_ALARM`) no — a parita' di `setOngoing`/`setFullScreenIntent`/`setLocalOnly` fra le due (nessuno dei tre presente su entrambe, dopo il fix della Causa 12). La differenza residua era canale/categoria. Vedi Causa 13.
13. **Causa 13 (v1.4.36, causa reale — non nella lista ufficiale delle 4 esclusioni Wear OS)**: `CH_ALARM` (`IMPORTANCE_HIGH`) + `CATEGORY_ALARM` impedisce il bridging su questo stack Mobvoi/TicWatch, mentre lo stesso contenuto su `CH_UPCOMING` (`IMPORTANCE_LOW`, nessuna categoria) bridgea regolarmente — comportamento non documentato nella pagina ufficiale Wear OS (che elenca solo `setLocalOnly`/`setOngoing`/`FLAG_NO_CLEAR`/bridging disattivato, vedi Causa 12), quindi probabilmente un filtro aggiuntivo lato OEM (es. Mobvoi tratta le notifiche `CATEGORY_ALARM` come riservate al proprio alarm nativo, per evitare un doppione).
    - **Fix**: `ringing()` ricostruita a partire dallo stesso builder di `showAlarmReminder()` — stesso canale `CH_UPCOMING`, niente `setCategory`/`setPriority`/`setVisibility` — con solo le due `addAction` (Posticipa/Spegni, icona valida `R.drawable.ic_stat_alarm` invece di `0`: alcune implementazioni Wear scartano le action senza icona) aggiunte sopra. Resta slegata dal foreground service come dalla Causa 12: `foregroundPlaceholder()` (mai bridgeata, `setLocalOnly(true)`) continua a usare `CH_ALARM` per l'allerta sul telefono, che non dipende dal bridging. `collectWearDiagnostics()` in `SettingsScreen.kt` aggiornato per leggere l'importanza di `CH_UPCOMING` (il canale ora rilevante) invece di `CH_ALARM`.
...
| `app/src/main/java/com/noalarm/alarm/NotificationHelper.kt` | Costruisce le notifiche sveglia; da v1.4.34 sono due oggetti separati — `ringing()` (bridgeabile, mai ongoing, da v1.4.36 su `CH_UPCOMING` come il preavviso) e `foregroundPlaceholder()` (lega il foreground service, sempre `setLocalOnly(true)`, resta su `CH_ALARM`) — vedi "Causa 12"/"Causa 13" |
...
**Ultima revisione**: v1.4.36 (18 Sep 2026) — prova reale dell'utente: il preavviso (`CH_UPCOMING`, nessuna categoria) bridgea sul TicWatch E3, la notifica di sveglia in corso (`CH_ALARM`, `CATEGORY_ALARM`) no, a parita' di `setOngoing`/`setFullScreenIntent`/`setLocalOnly` fra le due — causa non compresa nella lista ufficiale delle 4 esclusioni Wear OS (Causa 12), quindi probabile filtro OEM sulle notifiche `CATEGORY_ALARM`. Fix: `ringing()` ricostruita a partire dallo stesso builder di `showAlarmReminder()` (stesso canale `CH_UPCOMING`, niente categoria/priorita'/visibilita'), solo con le due azioni Posticipa/Spegni aggiunte sopra (icona valida invece di `0`). Vedi Causa 13. Non ancora riverificato su TicWatch E3.
