...
- **Causa concreta trovata in v1.4.30**: `NotificationHelper.ringing()` usava `setOngoing(true)` e `setFullScreenIntent(...)`, entrambi esclusi dal bridging di sistema di Wear OS — le notifiche `ongoing` non vengono bridgeate, e `fullScreenIntent` non e' supportato lato watch (viene comunque ignorato, ma teneva il builder "sporco"). **Fix**: rimossi entrambi. `setOngoing(true)` non serviva nemmeno sul telefono: la notifica e' legata a un foreground service (`AlarmService`), che il sistema rende gia' non sganciabile con uno swipe indipendentemente da questo flag. Per `setFullScreenIntent` invece serviva un sostituto, perche' era l'unico modo con cui `AlarmActivity` si apriva da sola sul telefono bloccato: **`AlarmReceiver.onReceive()`** (il `BroadcastReceiver` innescato da `AlarmManager`, quindi esente dai limiti BAL — stessa tecnica gia' usata per `WatchAlarmReceiver` in v1.2.0, vedi "Causa 11" sotto) ora chiama `context.startActivity()` direttamente su `AlarmActivity` invece di aspettare che lo faccia la notifica. Verificati anche gli altri punti noti per bloccare il bridging: nessun `setLocalOnly(true)` hardcoded (e' condizionale su `!alarm.ringOnWatch`, corretto), nessun meta-data `notificationBridgeMode` nel manifest.
...
**Ultima revisione**: v1.4.30 (17 Sep 2026) — trovata una causa concreta (non solo ipotesi) del bridging che non scatta: `setOngoing(true)` e `setFullScreenIntent(...)` sulla notifica sveglia, entrambi esclusi dal mirroring di sistema Wear OS. Rimossi; per non perdere l'apertura a schermo intero sul telefono bloccato, `AlarmReceiver` ora apre `AlarmActivity` con uno `startActivity()` diretto (esente BAL perche' innescato da AlarmManager). Verificati e confermati a posto gli altri punti noti (nessun `setLocalOnly(true)` hardcoded, nessun meta-data `notificationBridgeMode`). Ancora da testare sul TicWatch E3 se questo basta a far arrivare la notifica — vedi "Eco sul Watch via Notification Bridging di Sistema".
