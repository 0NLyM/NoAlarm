...
**Ultima revisione**: v1.4.33 (17 Sep 2026) — permesso di sistema e whitelist per-app in Mobvoi Health erano gia' entrambi attivi per NoAlarm: escluse anche queste ipotesi. Fatto nuovo: il watch riceve normalmente notifiche da altre app, quindi il bridging funziona in generale su questo hardware — il blocco e' specifico alla notifica di NoAlarm. Isolato stavolta `setOngoing(true)` da solo (tolto, `setFullScreenIntent` lasciato intatto per non rischiare di nuovo lo schermo intero sul telefono) — non era mai stato testato senza confonderlo con la rimozione di `setFullScreenIntent` insieme, come in v1.4.30. Vedi "Eco sul Watch via Notification Bridging di Sistema".
