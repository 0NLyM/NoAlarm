...
- **Prova reale (v1.4.26, utente su TicWatch E3): la notifica non arriva affatto sul watch.** Non e' un problema di full-screen vs. card come ipotizzato sopra — il bridging non scatta proprio. Ipotesi piu' probabile: sul Ticwatch il mirroring delle notifiche non e' il bridging nativo di Google Play Services (che richiederebbe comunque il pairing "Wear OS by Google" gia' rivelatosi assente con Mobvoi Health), ma un meccanismo separato implementato dall'app Mobvoi Health stessa via `NotificationListenerService` — che richiede il permesso "Accesso alle notifiche" concesso esplicitamente a Mobvoi Health in Impostazioni Android → App e notifiche → Accesso speciale alle app → Accesso alle notifiche (o l'equivalente switch dentro l'app Mobvoi Health). Senza quel permesso nessuna notifica di nessuna app arriva al watch, non solo quelle di NoAlarm — **prima cosa da controllare sul telefono dell'utente**. Se il permesso e' gia' concesso e ancora non arriva nulla, il bridging bluetooth di terze parti potrebbe semplicemente non includere `CATEGORY_ALARM`/notifiche `ongoing` nel suo mirroring (a differenza del bridging nativo Wear OS che le include sempre) — in tal caso l'unica via resta tornare al bridge Bluetooth custom (v1.4.25/v1.2.0, ancora in history).
- **Strumento di prova (v1.4.27, fix volume in v1.4.28)**: Impostazioni → sezione "Sveglia" → "Prova la sveglia adesso" fa suonare per davvero una sveglia sintetica (`AlarmScheduler.TEST_ID = -3L`, esclusa dall'elenco sveglie con `it.id >= 0` in `AlarmScreen.kt`) chiamando `AlarmService.ring()` direttamente — stessa notifica, stesso bridging, stesso `Posticipa`/`Spegni` di una sveglia vera, ma senza aspettare l'orario giusto. Utile per iterare rapidamente sul debug del bridging: se non compare sul watch nemmeno con questo, il problema e' nel bridging/permesso, non nello scheduling.
  - **v1.4.28**: la sveglia sintetica usava i default di `Alarm` — `gradualVolume = true` fa partire il volume al 5% e ci vuole ~45s per sentirla a pieno volume (`rampUp()` in `AlarmService.kt`, +0.02 ogni 900ms). Su un test rapido (suona/controlla/spegni in pochi secondi) sembrava "silenziata" invece che gradualmente crescente. **Fix**: `Alarm(id = TEST_ID, label = "Prova", gradualVolume = false)` — volume pieno da subito, solo per la sveglia di prova.
...
**Ultima revisione**: v1.4.28 (17 Sep 2026) — fix del volume nello strumento di prova (partiva al 5% per l'aumento graduale, sembrava silenziato) e prova reale confermata: il bridging di sistema (v1.4.26) continua a non far arrivare la notifica sul watch. Vedi "Eco sul Watch via Notification Bridging di Sistema" per l'ipotesi principale ancora da verificare (permesso "Accesso alle notifiche" per Mobvoi Health) e l'alternativa (tornare al bridge Bluetooth custom v1.4.25/v1.2.0).
