...
**Ultima revisione**: v1.4.29 (17 Sep 2026) — annullato il fix volume di v1.4.28: la sveglia di prova ora non suona ne' vibra affatto (`soundUri = SOUND_NONE`, `vibrate = false`), solo notifica/bridging/Glyph, per non essere disturbati ad ogni iterazione di debug. **Terza prova reale confermata dall'utente: il bridging di sistema (v1.4.26) ancora non fa arrivare la notifica sul watch**, nonostante lo strumento di prova confermi che la sveglia sul telefono funziona correttamente. Prossimo passo da concordare con l'utente: verificare il permesso "Accesso alle notifiche" per Mobvoi Health, oppure tornare al bridge Bluetooth custom (v1.4.25/v1.2.0) se il bridging di sistema si rivela semplicemente incompatibile con questo abbinamento watch-telefono di terze parti — vedi "Eco sul Watch via Notification Bridging di Sistema".
