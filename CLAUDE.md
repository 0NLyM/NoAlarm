...
- **Strumento di prova (v1.4.27, v1.4.29 senza audio/vibrazione)**: Impostazioni → sezione "Sveglia" → "Prova la sveglia adesso" fa suonare per davvero una sveglia sintetica (`AlarmScheduler.TEST_ID = -3L`, esclusa dall'elenco sveglie con `it.id >= 0` in `AlarmScreen.kt`) chiamando `AlarmService.ring()` direttamente — stessa notifica, stesso bridging, stesso `Posticipa`/`Spegni` di una sveglia vera, ma senza aspettare l'orario giusto. Utile per iterare rapidamente sul debug del bridging: se non compare sul watch nemmeno con questo, il problema e' nel bridging/permesso, non nello scheduling.
  ...
  - **Risultato reale (v1.4.32, utente su TicWatch E3): l'ipotesi del permesso mancante e' smentita.** Diagnosi raccolta: notifiche attive `true`, importanza canale `4` (alta, corretta), Bluetooth attivo `true`, e **`com.mobvoi.companion.at` e' presente** nell'elenco delle app con accesso alle notifiche — Mobvoi ha gia' il permesso di sistema. Tutti e 4 i controlli passano, eppure la notifica non arriva sul watch: il blocco non e' a nessuno dei livelli finora verificabili da codice. **Ipotesi successiva, non verificabile da codice**: molte app companion (Mobvoi Health incluso, tipicamente) hanno, oltre al permesso di sistema "Accesso alle notifiche", una **whitelist separata dentro l'app stessa** di quali app specifiche inoltrare al watch (schermata tipo "Notifiche" o "App notification sync" dentro Mobvoi Health) — NoAlarm, essendo un'app sconosciuta a Mobvoi, probabilmente non e' mai stata aggiunta a quella lista, a prescindere dal permesso Android concesso. Da controllare manualmente dall'utente dentro l'app Mobvoi Health/compagna del TicWatch, non ispezionabile da NoAlarm (nessuna API pubblica per leggere le impostazioni interne di un'altra app).
...
**Ultima revisione**: v1.4.32 (17 Sep 2026) — diagnosi raccolta sul dispositivo reale: notifiche attive, importanza canale e Bluetooth tutti ok, e **Mobvoi ha gia' il permesso di sistema "Accesso alle notifiche"** (`com.mobvoi.companion.at` presente nell'elenco) — l'ipotesi del permesso mancante e' smentita. Nessuna causa verificabile da codice resta aperta. Prossimo passo per l'utente: controllare dentro l'app Mobvoi Health se esiste una whitelist separata delle app da sincronizzare col watch (comune in questo genere di companion app) e se NoAlarm ci e' inclusa — vedi "Eco sul Watch via Notification Bridging di Sistema".
