...
class BridgeService : Service() {
            ...
            when (action) {
                ...
                ACTION_SYNC -> {
                    val entries = runCatching {
                        ...
                        (0 until count).map { Triple(input.readLong(), input.readLong(), input.readUTF()) }
                    }.getOrNull() ?: break
                    WatchAlarmScheduler.sync(this, entries)
                }
            }
        }
     ...
     * breve (non piu' per tutta la vita del servizio, vedi WatchAlarmScheduler)
     * tiene sveglia la CPU giusto per questa chiamata, cosi' il sistema esegue
     * subito l'avvio invece di rimandarlo mentre lo schermo e' spento - da li'
     * in poi ci pensano setShowWhenLocked/setTurnScreenOn di RingActivity ad
     * accendere lo schermo.
     */
    private fun ring(id: Long, label: String) {
        val wl = getSystemService(PowerManager::class.java)
            .newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "noalarm:bridge-ring")
